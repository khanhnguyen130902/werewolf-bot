...
async function runScenario(run: number): Promise<StressResult> {
  ...
  // Deterministic baseline for this fixed 6-player scenario. A mismatch is a
  // behavioral regression even if the game still reaches GAME_OVER.
  expect(events.length).toBe(38);
  expect(phaseHistory.filter((state) => state === GameState.DISCUSSION).length).toBe(1);
  expect(telemetry.observationCount).toBe(6);
  ...
  return {
    ...
    rounds: phaseHistory.filter((state) => state === GameState.DISCUSSION).length,
...
describe('bottest 100-round regression and stress flow', () => {
  it('passes 100 deterministic full-flow rounds and writes performance metrics', async () => {
    jest.useFakeTimers();
    ...
    const totalElapsedMs = elapsed.reduce((sum, value) => sum + value, 0);
      ...
      baselineMatches: results.filter((result) => result.eventCount === 38 && result.rounds === 1 && result.telemetryObservationCount === 6).length,
      totalElapsedMs,
    ...
    writeFileSync('bottest-stress-100-results.json', JSON.stringify(metrics, null, 2));
    expect(metrics.runs).toBe(100);
    expect(metrics.passedRuns).toBe(100);
    expect(metrics.baselineMatches).toBe(100);
