...
| `SILENT_MAGE_SILENCE` / `MUTE` | 🤫 Làm câm | Pháp sư câm | Đêm → Ngày | Living valid target | Silence duration/cycle stored in room state; dead-player and target rules enforced | One per configured night/cycle | Restricts target discussion according to `BotPolicy`/`DayService` | Invalid phase, invalid target, dead actor, duplicate action |
